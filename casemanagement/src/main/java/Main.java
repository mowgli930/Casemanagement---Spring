...
public class Main {
	...
	public static void main(String[] args) {
		...
		try (AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext()) {
			...
			WorkItem workItem = new WorkItem("heheh", WorkItem.Status.DONE)
					.setIssue(issue);
...
//			Slice<WorkItem> workItems = workItemRepository.getWorkItemsWithIssue(new PageRequest(0, 10));
			User user = User.builder().setActive(true).setFirstName("Analking").setLastName("Skywalker")
					.setTeam(team).setWorkItems(Arrays.asList(workItem)).build("username4");
			user = userRepository.save(user);
			Slice<WorkItem> workItems = workItemRepository.findByUserId(10L, new PageRequest(0, 10));
...
//			service.updateUserFirstName(4L, "wtf");
